...
include "../../../../library/circomlib/circuits/bitify.circom"; //引入 mimc hash算法
...
template OpenChannel(){
    ...
    get_root[0] = GetMerkleRoot(depth);
    get_root[0].leaf <== old_urcm[0];
    for(var j = 0; j < depth; j++){
        get_root[0].paths2_root[j] <== path_1[j];
        get_root[0].paths2_root_pos[j] <== n2b[0].out[j];
    }
    computed_root[0] <== get_root[0].out;
    
    get_root[1] = GetMerkleRoot(depth);
    get_root[1].leaf <== old_urcm[1];
    for(var j = 0; j < depth; j++){
        get_root[1].paths2_root[j] <== path_1[j];
        get_root[1].paths2_root_pos[j] <== n2b[1].out[j];
    }
    computed_root[1] <== get_root[1].out;
     
    computed_root[0] == computed_root[1];
    root <== computed_root[0];
}    
